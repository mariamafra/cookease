...
public class ReceitaService {
    ...
    public Receita getById(int id) {
        if(id < 0){
            ...
            Optional<Receita> receitaOpt = receitas.stream().filter(receita -> receita.getCodigo() == id).findFirst();
    ...
    private boolean resourceNotFound(Integer id) {
        return receitas.stream().filter(receita -> receita.getCodigo() == id).findFirst().isEmpty();
    }
}
